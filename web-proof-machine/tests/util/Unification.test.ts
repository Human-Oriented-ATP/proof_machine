...
test("Empty unification", () => {
    const emptyListOfEquations = new Map<EquationId, Equation>()
...
test("Conflict with different constants", () => {
    const equations = new Map<EquationId, Equation>([["eq1", parseEquation("1=2")]])
    const { equationIsSatisfied, assignment } = unifyEquations(equations)
    expect(Array.from(equationIsSatisfied)).toEqual([["eq1", false]])
    expect(assignment.getAssignedValues()).toEqual([])
})

test("Unification of a single variable", () => {
    const equations = new Map<EquationId, Equation>([["eq1", parseEquation("A=1")]])
    const { equationIsSatisfied, assignment } = unifyEquations(equations)
    expect(Array.from(equationIsSatisfied)).toEqual([["eq1", true]])
...
test("Unification of a single variable symmetric", () => {
    const equations = new Map<EquationId, Equation>([["eq1", parseEquation("1=B")]])
    const { equationIsSatisfied, assignment } = unifyEquations(equations)
    expect(Array.from(equationIsSatisfied)).toEqual([["eq1", true]])
    expect(assignment.getAssignedValues()).toEqual([parseTerm("1")])
...
test("Transitive unification of a single variable", () => {
    const equations = new Map<EquationId, Equation>([["eq1", parseEquation("A=B")], ["eq2", parseEquation("B=1")]])
    const { equationIsSatisfied, assignment } = unifyEquations(equations)
    expect(Array.from(equationIsSatisfied)).toEqual([["eq1", true], ["eq2", true]])
    expect(assignment.getAssignedValues()).toEqual([parseTerm("1")])
    expect(assignment.getAssignedValue("A")).toEqual(parseTerm("1"))
})

test("Conflict in transitive unification", () => {
    const equations = new Map<EquationId, Equation>([["eq1", parseEquation("2=B")], ["eq2", parseEquation("B=1")]])
    const { equationIsSatisfied, assignment } = unifyEquations(equations)
    expect(Array.from(equationIsSatisfied)).toEqual([["eq1", true], ["eq2", false]])
...
test("Valid circular assignment", () => {
    const equations = new Map<EquationId, Equation>([["eq1", parseEquation("A=B")], ["eq2", parseEquation("B=A")], ["eq3", parseEquation("A=1")]])
    const { equationIsSatisfied, assignment } = unifyEquations(equations)
    expect(Array.from(equationIsSatisfied)).toEqual([["eq1", true], ["eq2", true], ["eq3", true]])
    expect(assignment.getAssignedValues()).toEqual([parseTerm("1")])
    expect(assignment.getAssignedValue("B")).toEqual(parseTerm("1"))
})

test("Valid circular equivalence", () => {
    const equations = new Map<EquationId, Equation>([["eq1", parseEquation("A=B")], ["eq2", parseEquation("B=C")], ["eq3", parseEquation("C=A")]])
    const { equationIsSatisfied, assignment } = unifyEquations(equations)
    expect(Array.from(equationIsSatisfied)).toEqual([["eq1", true], ["eq2", true], ["eq3", true]])
    expect(assignment.getAssignedValues()).toEqual([])
...
test("Circular equality", () => {
    const equations = new Map<EquationId, Equation>([["eq1", parseEquation("f(A)=A")]])
    const { equationIsSatisfied, assignment } = unifyEquations(equations)
    expect(Array.from(equationIsSatisfied)).toEqual([["eq1", false]])
    expect(assignment.getAssignedValues()).toEqual([])
...
test("Circular equality with transitivity", () => {
    const equations = new Map<EquationId, Equation>([["eq1", parseEquation("f(A)=B")], ["eq2", parseEquation("A=B")]])
    const { equationIsSatisfied, assignment } = unifyEquations(equations)
    expect(Array.from(equationIsSatisfied)).toEqual([["eq1", true], ["eq2", false]])
...
test("Circular equality with transitivity symmetric", () => {
    const equations = new Map<EquationId, Equation>([["eq1", parseEquation("A=B")], ["eq2", parseEquation("f(A)=B")]])
    const { equationIsSatisfied, assignment } = unifyEquations(equations)
    expect(Array.from(equationIsSatisfied)).toEqual([["eq1", true], ["eq2", false]])
    expect(assignment.getAssignedValues()).toEqual([])
...
test("Circular equality with transitivity twice valid", () => {
    const equations = new Map<EquationId, Equation>([["eq1", parseEquation("A=B")], ["eq2", parseEquation("f(B)=C")], ["eq3", parseEquation("f(A)=C")]])
    const { equationIsSatisfied, assignment } = unifyEquations(equations)
    expect(Array.from(equationIsSatisfied)).toEqual([["eq1", true], ["eq2", true], ["eq3", true]])
    expect(assignment.findRepresentative("B")).toEqual(assignment.findRepresentative("A"))
    expect(assignment.findRepresentative("A")).toEqual(assignment.findRepresentative("B"))
...
test("Circular equality with transitivity twice invalid", () => {
    const equations = new Map<EquationId, Equation>([["eq1", parseEquation("A=B")], ["eq2", parseEquation("f(B)=C")], ["eq3", parseEquation("A=f(C)")]])
    const { equationIsSatisfied, assignment } = unifyEquations(equations)
    expect(Array.from(equationIsSatisfied)).toEqual([["eq1", true], ["eq2", true], ["eq3", false]])
